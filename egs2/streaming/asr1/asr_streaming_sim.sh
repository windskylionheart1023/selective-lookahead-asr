...
use_streaming_sim=true # Whether to use streaming simulation decoding (default true for this script)

# Streaming simulation parameters
# Note: sim_chunk_length auto-computed from chunk_size * subsampling_factor * hop_length
stream_chunk_size=8     # Chunk size for ChunkedMaskConfig (encoder attention)
stream_num_left_chunks=-1  # Historical chunks visible (-1 = unlimited)
stream_num_right_chunks=0  # Future chunks visible (0 = causal/streaming)
cross_attn_num_left_chunks=""  # Decoder cross-attn left chunks (empty = same as stream_num_left_chunks)
stream_num_global_tokens=0 # Number of global attention sink tokens
use_chunked_streaming=true # Use chunked streaming with delayed decoding
use_decoder_self_kvcache=true   # Use KV cache for decoder self-attention
use_decoder_cross_kvcache=true     # Use KV cache for decoder cross-attention
use_encoder_self_kvcache=false  # Stage B: enable proper incremental encoder self-attn KV cache
use_asymmetric_mask_at_inference=false  # C=2 inference: enable asymmetric mask + extract c_q=nrc slice
chunk_end_confidence_threshold=0.0    # Policy 1: softmax threshold for end-of-chunk early stop (0 = disabled)
chunk_start_confidence_threshold=0.0  # Policy 2: softmax threshold for start-of-chunk mask+retry (0 = disabled)
rollback_confidence_threshold=0.0     # Policy 3: softmax threshold for rollback + agreement confirm at chunk boundary (0 = disabled)
force_dcconv_right_context_at_inference=false  # Diagnostic: undo causal DCConv at inference (train/test mismatch, ablation only)
monotonic_attn_floor=-1                         # Monotonic cross-attn floor in frames; -1 disables. Typical: 4-8
score_token_types="char word bpe"     # Stage 13 scoring levels; e.g. "word" for WER-only sweeps
stage_data_to_scratch=false           # Stage 12: copy audio + model to node-local scratch first (unstable NFS)
...
Options:
    ...
    --use_streaming_sim   # Whether to use streaming simulation decoding (default="${use_streaming_sim}").

    # Streaming simulation parameters (sim_chunk_length auto-computed, no CLI arg needed)
    --stream_chunk_size        # Chunk size for encoder attention (default="${stream_chunk_size}").
    --stream_num_left_chunks   # Historical chunks visible, -1=unlimited (default="${stream_num_left_chunks}").
    --stream_num_right_chunks  # Future chunks visible, 0=causal (default="${stream_num_right_chunks}").
    --cross_attn_num_left_chunks # Decoder cross-attn left chunks (default=same as stream_num_left_chunks).
    --stream_num_global_tokens # Number of global attention sink tokens (default="${stream_num_global_tokens}").
    --use_chunked_streaming    # Use chunked streaming with delayed decoding (default="${use_chunked_streaming}").
    --use_decoder_self_kvcache      # Use KV cache for decoder self-attention (default="${use_decoder_self_kvcache}").
    --use_decoder_cross_kvcache        # Use KV cache for decoder cross-attention (default="${use_decoder_cross_kvcache}").
    --use_encoder_self_kvcache   # Stage B: incremental encoder self-attn KV cache (default="${use_encoder_self_kvcache}").
    --chunk_end_confidence_threshold   # Policy 1 threshold; 0 disables (default="${chunk_end_confidence_threshold}").
    --chunk_start_confidence_threshold # Policy 2 threshold; 0 disables (default="${chunk_start_confidence_threshold}").
    --rollback_confidence_threshold    # Policy 3 threshold; 0 disables (default="${rollback_confidence_threshold}").
    --use_asymmetric_mask_at_inference # Use the asymmetric chunk mask at inference (default="${use_asymmetric_mask_at_inference}").
    --force_dcconv_right_context_at_inference  # Diagnostic flag (default="${force_dcconv_right_context_at_inference}").
    --monotonic_attn_floor     # Monotonic cross-attn floor in frames, -1 disables (default="${monotonic_attn_floor}").
    --score_token_types        # Stage 13 scoring levels, subset of "char word bpe" (default="${score_token_types}").
    --stage_data_to_scratch    # Copy audio + model to node-local scratch before decoding (default="${stage_data_to_scratch}").
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    ...
    if [ ${asr_task} == "asr" ]; then
        ...
        elif "${use_streaming_sim}"; then
            inference_bin_tag="_streaming_modified"
            # Add streaming simulation specific options
            _opts+="--use_chunked_streaming ${use_chunked_streaming} "
            _opts+="--chunk_size ${stream_chunk_size} "
            _opts+="--num_left_chunks ${stream_num_left_chunks} "
            _opts+="--num_right_chunks ${stream_num_right_chunks} "
            if [ -n "${cross_attn_num_left_chunks}" ]; then
                _opts+="--cross_attn_num_left_chunks ${cross_attn_num_left_chunks} "
            fi
            _opts+="--num_global_tokens ${stream_num_global_tokens} "
            _opts+="--use_decoder_self_kvcache ${use_decoder_self_kvcache} "
            _opts+="--use_decoder_cross_kvcache ${use_decoder_cross_kvcache} "
            _opts+="--use_encoder_self_kvcache ${use_encoder_self_kvcache} "
            _opts+="--use_asymmetric_mask_at_inference ${use_asymmetric_mask_at_inference} "
            _opts+="--chunk_end_confidence_threshold ${chunk_end_confidence_threshold} "
            _opts+="--chunk_start_confidence_threshold ${chunk_start_confidence_threshold} "
            _opts+="--rollback_confidence_threshold ${rollback_confidence_threshold} "
            _opts+="--force_dcconv_right_context_at_inference ${force_dcconv_right_context_at_inference} "
            _opts+="--monotonic_attn_floor ${monotonic_attn_floor} "
    ...
    # Optionally stage the model + data on node-local scratch so decoding
    # does not depend on NFS staying healthy (audioslave instability).
    # local/stage_to_scratch.sh degrades gracefully: on any staging failure
    # it returns the original NFS path. Under Condor the scratch dir is
    # auto-cleaned; otherwise we remove ${STAGE_ROOT} after this stage.
    _asr_train_config="${asr_exp}/config.yaml"
    _asr_model_file="${asr_exp}/${inference_asr_model}"
    if "${stage_data_to_scratch}"; then
        export STAGE_ROOT="${_CONDOR_SCRATCH_DIR:-${TMPDIR:-/tmp}}/espnet_stage_$$"
        _asr_train_config=$(local/stage_to_scratch.sh --file "${_asr_train_config}")
        _asr_model_file=$(local/stage_to_scratch.sh --file "${_asr_model_file}")
    fi

    for dset in ${_dsets}; do
        _data="${data_feats}/${dset}"
        if "${stage_data_to_scratch}"; then
            _data=$(local/stage_to_scratch.sh "${_data}")
        fi
        ...
        ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
            ${python} -m espnet2.bin.${asr_task}_inference${inference_bin_tag} \
                ...
                --asr_train_config "${_asr_train_config}" \
                --asr_model_file "${_asr_model_file}" \
                --output_dir "${_logdir}"/output.JOB \
                ${_opts} ${inference_args} || { cat $(grep -l -i error "${_logdir}"/asr_inference.*.log) ; exit 1; }

        # 3. Calculate and report RTF based on decoding logs. Also runs for
        # the chunked streaming bin, which logs the same "speech length" /
        # "best hypo" marker pair per utterance.
        if [ ${asr_task} == "asr" ] && { [ -z "${inference_bin_tag}" ] || [ "${inference_bin_tag}" = "_streaming_modified" ]; }; then
            ...
            ${_cmd} JOB=1 "${_logdir}"/calculate_rtf.log \
                pyscripts/utils/calculate_rtf.py \
                    ...
                    --inf-num ${num_inf} \
                || log "Warning: RTF calculation failed (see '${_logdir}/calculate_rtf.log'); decode outputs are unaffected."
        fi
    ...
    # Remove staged copies (no-op under Condor scratch, which auto-cleans).
    if "${stage_data_to_scratch}" && [ -n "${STAGE_ROOT:-}" ]; then
        rm -rf "${STAGE_ROOT}"
    fi
fi
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ] && ! [[ " ${skip_stages} " =~ [[:space:]]13[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        # shellcheck disable=SC2086
        for _tok_type in ${score_token_types}; do
            ...
            for ref_txt in ${ref_text_files[@]}; do
                ...
                # Tokenize text to ${_tok_type} level. The tokenized reference
                # is identical for every inference_tag of a dataset, so cache
                # it per (ref file, token type, scoring options); the key
                # hashes the options and input contents, so any change
                # re-tokenizes. Writes are atomic (tmp + mv) for concurrent
                # decodes of the same dataset.
                _ref_cache_dir="${_data}/.ref_trn_cache"
                mkdir -p "${_ref_cache_dir}"
                _ref_key=$({
                    echo "cleaner=${cleaner} opts=${_opts}"
                    md5sum "${_data}/${ref_txt}" "${_data}/utt2spk"
                    if [ "${_tok_type}" = bpe ]; then md5sum "${bpemodel}"; fi
                } | md5sum | cut -d' ' -f1)
                _ref_cache="${_ref_cache_dir}/ref${suffix}.${_type}.${_ref_key}.trn"
                if [ ! -f "${_ref_cache}" ]; then
                    paste \
                        <(<"${_data}/${ref_txt}" \
                            ${python} -m espnet2.bin.tokenize_text  \
                                -f 2- --input - --output - \
                        ...
                        <(<"${_data}/utt2spk" awk '{ print "(" $2 "-" $1 ")" }') \
                            >"${_ref_cache}.tmp$$"
                    mv -f "${_ref_cache}.tmp$$" "${_ref_cache}"
                fi
                cp "${_ref_cache}" "${_scoredir}/ref${suffix:-${suffix}}.trn"
