...
use_alignment=false # Use alignment data for streaming ASR training
alignment_file=token_frame_mapping # Name of the alignment file in data dir
...
Options:
    ...
    --use_alignment  # Whether to use alignment for streaming ASR training (default="${use_alignment}").
    --alignment_file # Name of the alignment file in data dir (default="${alignment_file}").
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ] && ! [[ " ${skip_stages} " =~ [[:space:]]10[[:space:]] ]]; then
    ...
    if ${use_alignment}; then
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${alignment_file},alignment,text_int "
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/${alignment_file},alignment,text_int "
    fi
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ] && ! [[ " ${skip_stages} " =~ [[:space:]]11[[:space:]] ]]; then
    ...
    if ${use_alignment}; then
        _opts+="--fold_length ${asr_text_fold_length} "
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${alignment_file},alignment,text_int "
        _opts+="--train_shape_file ${asr_stats_dir}/train/alignment_shape "
        _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/${alignment_file},alignment,text_int "
        _opts+="--valid_shape_file ${asr_stats_dir}/valid/alignment_shape "
    fi
